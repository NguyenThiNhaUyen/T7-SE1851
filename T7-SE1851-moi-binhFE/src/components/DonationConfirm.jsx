...
const DonationConfirm = () => {
  ...
  form.validateFields().then(values => {
    ...
    // 🔍 GỌI API để lấy bloodBagId từ registrationId
    axios.get(`${API_BASE}/api/blood-bags/by-registration`, {
      params: { registrationId },
      headers: getAuthHeader()
    }).then(res => {
      const bloodBagId = res.data.bloodBagId;
      if (!bloodBagId) {
        notification.error({
      ...
      // ✅ GỌI API TÁCH MÁU
      axios.post(`${API_BASE}/api/separation-orders/create-basic`, null, {
        headers: getAuthHeader(),
        params: {
          bloodBagId,
          volume,
  ...
  const handleViewHealthCheck = (record) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        form.setFieldsValue(res.data);            // Nếu dùng Form để nhập
        setHealthModalVisible(true);              // ✅ Mở modal sau khi có dữ liệu
      })
      .catch((err) => {
      ...
      render: (text, record) => (
        <>
          ...
          <Button
            ...
            type="link"
            style={{ color: '#52c41a' }}
            onClick={() => handleCreateHealthCheck(record)}
          >
            Tạo phiếu
          </Button>
        </>
        ...
        {healthFormLoading ? (
          ...
          <Form form={form} layout="vertical">
            ...
            <Row gutter={16}>
              ...
              <Col span={12}>
                <Form.Item name="isEligible" label="Đủ điều kiện hiến máu?" valuePropName="checked">
                  <Switch />
                </Form.Item>
              </Col>
            </Row>
